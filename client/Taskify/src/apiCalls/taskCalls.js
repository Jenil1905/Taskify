        ...
        <div className="mt-1 flex items-center gap-2 text-xs text-gray-500">
          ...
          {task.due && (
            <span className="inline-flex items-center gap-1">
              <Calendar size={14} /> {new Date(task.due).toLocaleDateString()}
            </span>
          )}
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'dueAsc') {
      arr.sort((a, b) => new Date(a.due) - new Date(b.due));
    } else if (sortBy === 'dueDesc') {
      arr.sort((a, b) => new Date(b.due) - new Date(a.due));
        ...
        <form id="task-form" onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              name="isDone"
              type="checkbox"
              defaultChecked={editing?.isDone || false}
              className="w-5 h-5 accent-indigo-600"
            />
